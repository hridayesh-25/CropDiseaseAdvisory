...
const predictDisease = async (diseaseName, imagePath) => {
  ...
  const commonDiseases = {
    ...
    rust: { name: "Rust Disease", confidence: 0.8 },
    blight: { name: "Blight", confidence: 0.75 },
    "powdery mildew": { name: "Powdery Mildew", confidence: 0.9 },
...
router.get("/", auth, async (req, res) => {
  try {
    ...
    if (req.user.role === "user") {
      ...
      // Admins can see all diseases, optionally filtered by status
      if (status) {
        query.status = status;
      }
    }

    // Allow status override from query if user has permission
...
router.get("/:id", auth, async (req, res) => {
  try {
    ...
    if (
      req.user.role === "user" &&
      disease.user._id.toString() !== req.user.id
    ) {
    ...
    if (medicines && Array.isArray(medicines) && medicines.length > 0) {
      ...
      const foundMedicines = await Medicine.find({
        $or: [
          {
            disease: {
              $regex: disease.predictedDisease || disease.diseaseName,
              $options: "i",
            },
          },
...
// @desc    Update disease (status, medicines) - Admin/Specialist
// @access  Private/Admin/Specialist
router.put("/:id", auth, async (req, res) => {
  try {
    const { status, medicines, specialistNotes } = req.body;

    // Check authorization - allow admin or specialist
